...
- **The markdown stays plain; the builder adds the structure** from the shape it already has, so the page is not a wall of text. Each `##` chapter becomes a tab, and each `###` becomes a card (techniques, subjects, formats, profiles) or a topic. A callout's label sets its kind (ADHD, tip, caution, note). "How to apply it." becomes a panel, with numbered steps. "Good for:" becomes tags, and "Myth:" paragraphs become a marked list. The finder's tables become a clickable finder, the review-calendar table a timeline, and the cheat sheet a grid with stop/start cards. Which chapters use cards is a small table at the top of the builder (`CHAPTERS`, `PLAIN_SECTIONS`); a test fails if it names a heading the guide no longer has.
- **It degrades cleanly.** Without JavaScript every chapter shows in one scroll and the finder's tables stand in for it. In Chrome/Edge, find-in-page still reaches hidden chapters (`hidden="until-found"`), and "Whole guide" shows everything at once. Printing prints every chapter, with the cheat sheet on a page of its own; the cheat sheet's button prints just that page.
